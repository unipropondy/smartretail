// components/SettlementReport.tsx - FINAL PERFECT VERSION
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [openingRes, cashOutRes, physicalRes, salesRes, voidRes, checkRes] = await Promise.all([
          API.get(`/settlement/opening-cash?outletId=${outletId}&date=${dateStr}`),
          API.get(`/settlement/cash-out?outletId=${outletId}&date=${dateStr}`),
          API.get(`/settlement/physical-cash?outletId=${outletId}&date=${dateStr}`),
          API.get(`/sales?filter=custom&startDate=${dateStr}&endDate=${dateStr}`),
          API.get(`/sales?filter=custom&startDate=${dateStr}&endDate=${dateStr}&status=voided`).catch(() => ({ data: [] })),
          API.get(`/settlement/check?outletId=${outletId}&date=${dateStr}`)
        ]);
        
        // Process opening cash
        if (openingRes.data.success && openingRes.data.data) {
          setOpeningNotes(openingRes.data.data.notes || openingNotes);
          setOpeningCoins(openingRes.data.data.coins || openingCoins);
          setCashFlow(prev => ({ ...prev, openingCash: openingRes.data.data.total || 0 }));
        ...
        if (cashOutRes.data.success && cashOutRes.data.data) {
          ...
          const total = cashOutRes.data.data.reduce((sum, item) => sum + item.amount, 0);
          setCashFlow(prev => ({ ...prev, manualCashOutTotal: total }));
        }
        
        // Process physical cash
        if (physicalRes.data.success && physicalRes.data.data) {
          setPhysicalNotes(physicalRes.data.data.notes || physicalNotes);
          setPhysicalCoins(physicalRes.data.data.coins || physicalCoins);
          const total = physicalRes.data.data.total || 0;
          setCashFlow(prev => ({ ...prev, physicalCash: total }));
          setPhysicalCashSaved(true);
        }
        
        // Process sales data
        // Process sales data - FIXED for nested discount
...
sales.forEach((sale: any) => {
  ...
  if (sales.indexOf(sale) < 3) {
    console.log(`🔍 Sale ${sale.id}:`, {
      ...
      discountRoot: sale.discountAmount
    });
  }
  
  // Payment breakdown using final amount
...
voidSales.forEach((sale: any) => {
  ...
  // Extract discount for voided sales too
  if (sale.discount && sale.discount.amount) {
...
console.log('📊 Settlement calculated:', { totalSales, totalDiscount, voidAmount });
...
const saveOpeningCashToDatabase = async () => {
  ...
  await withMinDisplayTime(async () => {
    ...
    const currentCashReceived = cashFlow.cashReceived;
    const currentManualCashOut = cashFlow.manualCashOutTotal;
    const currentPhysicalCash = cashFlow.physicalCash;
    
    // ✅ Recalculate expected closing and variance
    const newExpectedClosing = total + currentCashReceived - currentManualCashOut;
    const newVariance = currentPhysicalCash - newExpectedClosing;
    
    // ✅ Update all related states
    setCashFlow(prev => ({ 
      ...prev, 
      openingCash: total,
      expectedClosing: newExpectedClosing,
  ...
  const onDateChange = useCallback((event: any, selectedDate?: Date) => {
    if (event.type === 'set' && selectedDate) {
      ...
      hasLoadedRef.current = false;
       setIsSettled(false);  // ✅ Reset settled status when date changes
...
useEffect(() => {
  if (visible) {
    // ✅ Reset settled status when modal opens
    setIsSettled(false);
    setSettlementId(null);
    hasLoadedRef.current = false;
  }
  ...
  const generateHTML = (): string => {
    ...
    <body>
        <div class="container">
            <div class="header">
                <h2>${outletName}</h2>
                <p>Cashier: ${cashierName} | Date: ${dateStr}</p>
...
const handleEmailReport = async () => {
    ...
    try {
        ...
        // Generate CSV content as string (NOT Buffer)
        const csvContent = generateCSV();
        
        // ✅ FIX: Convert string to base64 without Buffer
        // React Native alternative
        let excelBase64 = '';
        try {
            // Method 1: Using btoa (works in React Native)
            excelBase64 = btoa(csvContent);
        } catch (e) {
            // Method 2: Manual conversion if btoa fails
...
const stringToBase64 = (str: string): Promise<string> => {
    return new Promise((resolve, reject) => {
        try {
            // React Native's built-in base64 encoding
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      ...
      <View style={[styles.container, { backgroundColor: theme.background }]}>
        <View style={[styles.header, { backgroundColor: theme.primary }]}>
          <TouchableOpacity onPress={onClose} style={styles.backBtn}><Ionicons name="arrow-back" size={24} color="#fff" /></TouchableOpacity>
...
>
            ...
            <TouchableOpacity style={[styles.smallBtn, { backgroundColor: theme.danger, opacity: isSettled ? 0.6 : 1 }]} onPress={() => { setEditingCashOut(null); setCashOutAmount(''); setCashOutReason(''); setCashOutRecipient(''); setShowCashOutModal(true); }} disabled={isSettled}>
              <Ionicons name="remove-circle-outline" size={18} color="#fff" />
              <Text style={styles.smallBtnText}>Cash Out</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.smallBtn, { backgroundColor: theme.success, opacity: isSettled ? 0.6 : 1 }]} onPress={() => setShowPhysicalCashModal(true)} disabled={isSettled}>
          ...
          {loading ? <ActivityIndicator size="large" color={theme.primary} style={{ marginTop: 40 }} /> : (
            <>
              <View style={styles.summaryGrid}>
                <View style={[styles.summaryCard, { backgroundColor: theme.surface }]}><Text style={[styles.summaryLabel, { color: theme.textSecondary }]}>Total Sales</Text><Text style={[styles.summaryValue, { color: theme.primary }]}>{formatPrice(summary.totalSales)}</Text></View>
                <View style={[styles.summaryCard, { backgroundColor: theme.surface }]}><Text style={[styles.summaryLabel, { color: theme.textSecondary }]}>Discount</Text><Text style={[styles.summaryValue, { color: theme.danger }]}>-{formatPrice(summary.totalDiscount)}</Text></View>
                <View style={[styles.summaryCard, { backgroundColor: theme.surface }]}><Text style={[styles.summaryLabel, { color: theme.textSecondary }]}>Void</Text><Text style={[styles.summaryValue, { color: theme.danger }]}>-{formatPrice(summary.voidAmount)}</Text></View>
              </View>
              
              <View style={[styles.netCard, { backgroundColor: theme.primary }]}><Text style={styles.netLabel}>NET SALES</Text><Text style={styles.netValue}>{formatPrice(summary.netSales)}</Text></View>
              
              <View style={[styles.section, { backgroundColor: theme.surface }]}>
                <Text style={[styles.sectionTitle, { color: theme.text }]}>💰 Cash Flow</Text>
                <View style={styles.cashRow}><Text style={[styles.cashLabel, { color: theme.textSecondary }]}>Opening Cash</Text><Text style={[styles.cashValue, { color: theme.success }]}>{formatPrice(cashFlow.openingCash)}</Text>{openingCashSaved && <Text style={styles.savedIcon}>✓</Text>}</View>
                <View style={styles.cashRow}><Text style={[styles.cashLabel, { color: theme.textSecondary }]}>Cash Received</Text><Text style={[styles.cashValue, { color: theme.success }]}>+{formatPrice(cashFlow.cashReceived)}</Text></View>
                {manualCashOuts.map(item => (
                  <View key={item.id} style={[styles.cashRow, styles.manualRow]}>
                    <Text style={[styles.cashLabel, { color: theme.textSecondary, marginLeft: 20 }]}>➖ {item.reason}{item.recipient ? ` (${item.recipient})` : ''}</Text>
                    <Text style={[styles.cashValue, { color: theme.danger }]}>-{formatPrice(item.amount)}</Text>
                    {!isSettled && (
                      <View style={styles.cashOutActions}>
                        <TouchableOpacity onPress={() => editCashOut(item)}><Ionicons name="pencil" size={16} color={theme.primary} /></TouchableOpacity>
                        <TouchableOpacity onPress={() => deleteCashOut(item.id)}><Ionicons name="trash" size={16} color={theme.danger} /></TouchableOpacity>
                      </View>
                    )}
                  </View>
                ))}
                <View style={[styles.cashRow, styles.closingRow]}><Text style={[styles.cashLabel, { color: theme.text, fontWeight: 'bold' }]}>Expected Closing Cash</Text><Text style={[styles.cashValue, { color: theme.primary, fontWeight: 'bold', fontSize: 18 }]}>{formatPrice(cashFlow.expectedClosing)}</Text></View>
              </View>
              
              {cashFlow.physicalCash > 0 && (
                <View style={[styles.section, { backgroundColor: theme.surface }]}>
                  <Text style={[styles.sectionTitle, { color: theme.text }]}>🔍 Physical Cash Verification</Text>
                  <View style={styles.cashRow}><Text style={[styles.cashLabel, { color: theme.textSecondary }]}>Physical Cash Count</Text><Text style={[styles.cashValue, { color: theme.text }]}>{formatPrice(cashFlow.physicalCash)}</Text>{physicalCashSaved && <Text style={styles.savedIcon}>✓</Text>}</View>
                  <View style={[styles.cashRow, styles.varianceRow]}>
                    <Text style={[styles.cashLabel, { color: cashFlow.variance === 0 ? theme.success : cashFlow.variance > 0 ? theme.warning : theme.danger, fontWeight: 'bold' }]}>
                      {cashFlow.variance === 0 ? '✅ Balanced' : cashFlow.variance > 0 ? '💰 Over' : '⚠️ Short'}
                    </Text>
                    <Text style={[styles.cashValue, { color: cashFlow.variance === 0 ? theme.success : cashFlow.variance > 0 ? theme.success : theme.danger, fontWeight: 'bold', fontSize: 18 }]}>
              ...
              {isSettled ? (
                <View style={styles.actionGrid}>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.primary }]} onPress={handlePrint}><Ionicons name="print" size={20} color="#fff" /><Text style={styles.actionBtnText}>Print</Text></TouchableOpacity>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.secondary }]} onPress={handleDownloadPDF}><Ionicons name="download" size={20} color="#fff" /><Text style={styles.actionBtnText}>PDF</Text></TouchableOpacity>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.secondary }]} onPress={handleExportExcel}><Ionicons name="document-text" size={20} color="#fff" /><Text style={styles.actionBtnText}>Excel</Text></TouchableOpacity>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.warning }]} onPress={() => setShowEmailModal(true)}><Ionicons name="mail" size={20} color="#fff" /><Text style={styles.actionBtnText}>Email</Text></TouchableOpacity>
                </View>
              ) : (
                <TouchableOpacity style={[styles.endDayBtn, { backgroundColor: theme.danger, opacity: (!openingCashSaved || !physicalCashSaved) ? 0.5 : 1 }]} onPress={finalizeSettlement} disabled={!openingCashSaved || !physicalCashSaved}>
          ...
          <View style={[styles.modalContent, { backgroundColor: theme.card, maxHeight: '85%' }]}>
            <ScrollView showsVerticalScrollIndicator={true}>
              ...
              <View style={styles.modalButtons}>
                <TouchableOpacity style={[styles.modalCancelBtn, { borderColor: theme.border }]} onPress={() => setShowOpeningCashModal(false)}><Text style={[styles.modalCancelText, { color: theme.text }]}>Cancel</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.modalSaveBtn, { backgroundColor: theme.primary }]} onPress={saveOpeningCashToDatabase} disabled={processing}><Text style={styles.modalSaveText}>Save Opening Cash</Text></TouchableOpacity>
              </View>
            </ScrollView>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: theme.card, maxHeight: '85%' }]}>
            <ScrollView showsVerticalScrollIndicator={true} contentContainerStyle={{ paddingBottom: 20 }}>
              ...
              <TextInput style={[styles.modalInput, { backgroundColor: theme.surface, borderColor: theme.border, color: theme.text }]} placeholder="0.00" keyboardType="numeric" value={cashOutAmount} onChangeText={setCashOutAmount} />
              <Text style={[styles.modalLabel, { color: theme.textSecondary }]}>Reason *</Text>
              <TextInput style={[styles.modalInput, { backgroundColor: theme.surface, borderColor: theme.border, color: theme.text }]} placeholder="e.g., Supplier payment, Staff advance" value={cashOutReason} onChangeText={setCashOutReason} multiline numberOfLines={3} />
              <Text style={[styles.modalLabel, { color: theme.textSecondary }]}>Paid To (Optional)</Text>
              <TextInput style={[styles.modalInput, { backgroundColor: theme.surface, borderColor: theme.border, color: theme.text }]} placeholder="Recipient name" value={cashOutRecipient} onChangeText={setCashOutRecipient} />
              <View style={styles.modalButtons}>
                <TouchableOpacity style={[styles.modalCancelBtn, { borderColor: theme.border }]} onPress={() => { setShowCashOutModal(false); setEditingCashOut(null); }}><Text style={[styles.modalCancelText, { color: theme.text }]}>Cancel</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.modalSaveBtn, { backgroundColor: theme.danger }]} onPress={saveCashOutToDatabase} disabled={processing}><Text style={styles.modalSaveText}>{editingCashOut ? 'Update' : 'Add'}</Text></TouchableOpacity>
              </View>
            </ScrollView>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: theme.card, maxHeight: '90%' }]}>
            <ScrollView>
              ...
              <View style={styles.modalButtons}>
                <TouchableOpacity style={[styles.modalCancelBtn, { borderColor: theme.border }]} onPress={() => setShowPhysicalCashModal(false)}><Text style={[styles.modalCancelText, { color: theme.text }]}>Cancel</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.modalSaveBtn, { backgroundColor: theme.primary }]} onPress={savePhysicalCashToDatabase} disabled={processing}><Text style={styles.modalSaveText}>Verify & Save</Text></TouchableOpacity>
              </View>
            </ScrollView>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  processingContainer: { padding: 30, borderRadius: 20, alignItems: 'center', minWidth: 200, elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84 },

...
export default function PosScreen() {
  ...
  const loadStaffInfo = async () => {
    try {
      ...
      const settingsRes = await API.get('/staff/settings/info');
      if (settingsRes.data) {
        setIsStaffMandatory(settingsRes.data.isStaffMandatory);
      }
    } catch (err) {
  ...
  const handleCheckout = (): void => {
    ...
    try {
      ...
      // Reset selected staff on checkout start
      setSelectedStaff(null);

      // Show payment modal or staff selection modal if mandatory
      if (isStaffMandatory) {
  ...
  const renderMainMenu = () => (
    <View style={[styles.menuContent, { backgroundColor: currentTheme.background }]}>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Ionicons name="people-outline" size={20} color={currentTheme.text} />
          <Text style={[styles.menuItemBtnText, { color: currentTheme.text, marginLeft: 8 }]}>
            Staff Management
          </Text>
        </View>
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: currentTheme.background }]}>
      ...
      {showStaffManagement && (
        <StaffManagement
          ...
          onStaffUpdate={loadStaffInfo}
          currentTheme={currentTheme}
          t={t}
        />
      )}
      {/* Staff Selection Modal during checkout */}
      <Modal
        visible={showStaffSelectionModal}
        transparent={true}
        animationType="fade"
        onRequestClose={() => setShowStaffSelectionModal(false)}
      >
        <View style={[styles.modalOverlay, { justifyContent: 'center', alignItems: 'center' }]}>
          <View style={[styles.staffSelectionContainer, { backgroundColor: currentTheme.card }]}>
            <View style={[styles.staffSelectionHeader, { borderBottomColor: currentTheme.border }]}>
              <Text style={[styles.staffSelectionTitle, { color: currentTheme.text }]}>Select Staff Member</Text>
              <TouchableOpacity onPress={() => setShowStaffSelectionModal(false)} style={styles.closeButton}>
                <Ionicons name="close" size={24} color={currentTheme.textSecondary} />
              </TouchableOpacity>
            </View>

            {staffList.length === 0 ? (
              <View style={styles.staffSelectionEmpty}>
                <Text style={{ color: currentTheme.textSecondary, marginBottom: 12 }}>No staff members found.</Text>
                <TouchableOpacity
                  style={[styles.staffSelectionAddBtn, { backgroundColor: currentTheme.primary }]}
                  onPress={() => {
                    setShowStaffSelectionModal(false);
                    setShowStaffManagement(true);
                  }}
                >
                  <Text style={{ color: '#fff', fontWeight: 'bold' }}>Manage Staff</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <ScrollView style={styles.staffSelectionList}>
                {staffList.map((staff) => (
                  <TouchableOpacity
                    key={staff.Id}
                    style={[styles.staffSelectionItem, { backgroundColor: currentTheme.surface, borderBottomColor: currentTheme.border }]}
                    onPress={() => {
                  ...
                  >
                    <View style={[styles.staffSelectionAvatar, { backgroundColor: currentTheme.primary + '20' }]}>
                      <Text style={[styles.staffSelectionAvatarText, { color: currentTheme.primary }]}>
                        {staff.Name.substring(0, 2).toUpperCase()}
                      </Text>
                    </View>
                    <Text style={[styles.staffSelectionName, { color: currentTheme.text }]}>{staff.Name}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
